...
directive('spMostEntitlementsUserWidget', function() {
	   //console.log("Directive");
	    return {
	        ...
	        controller: 'MostEntitlementsUserWidgetDirectiveCtrl',
	        controllerAs: 'ctrl',
	        ...
	        template:
	            ...
	            '      </div>' +
	            '      <div class="list-group-item" ng-repeat="entUser in ctrl.items track by $index">' +
	            '        <div class="row" tabindex="-1" id="seriWidgetRow{{ $index }}">' +
	            '          <div class="col-sm-7">' +
	            '            <a href tabindex="50" ng-click="ctrl.viewIdentity(entUser)" ' +
	            '              <span class="text-ellipsis">{{ ::entUser.displayName }}</span>' +
	            '            </a>' +
	            '          </div>' +
	            '          <div class="col-sm-5 text-right">' +
	            '             <span class="text-ellipsis">{{ ::entUser.count }}</span>' +
	            '          </div>' +
	            '        </div>' +
	            '      </div>' +
	            '    </div>' +
