...
function MostEntitlementsUserWidgetDirectiveCtrl(SearchData, PageState, PagingData, $q, $timeout, configService, mostEntitlementsUserService, navigationService, ListResultDTO, ListResultCache) {
    ...
    function loadIntoCache(searchTerm, startIdx) {
        ...
        return mostEntitlementsUserService.getMostEntitlementsUsers(searchTerm, startIdx, CACHE_CHUNK_SIZE).then(function (users) {
     ...
     */
    this.viewIdentity = function (entUser) {
    	//console.log("Calling viewIdentity");
    	//console.log(entUser);
    	//console.log(entUser.id);
        navigationService.go({
            outcome: 'viewIdentity#/identities/' + entUser.id + '/attributes'
        });
    };
    /**
...
MostEntitlementsUserWidgetDirectiveCtrl.$inject = ['SearchData', 'PageState', 'PagingData', '$q', '$timeout', 'configService', 'mostEntitlementUsersService', 'navigationService', 'ListResultDTO', 'ListResultCache'];

SailPoint.extend(MostEntitlementsUserWidgetDirectiveCtrl, AbstractListCtrl);
angular.module('sailpoint.home.widget').controller('MostEntitlementsUserWidgetDirectiveCtrl', MostEntitlementsUserWidgetDirectiveCtrl);
})();
