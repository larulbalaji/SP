...
directive('spIIQStatsWidget', function() {
	   //console.log("Directive");
	    return {
	        ...
	        template:
	            ...
	            '      </div>' +
	            '      <div class="list-group-item" ng-repeat="stat in ctrl.items track by $index">' +
	            '        <div class="row" tabindex="-1" id="seriWidgetRow{{ $index }}">' +
	            '          <div class="col-sm-7">' +
	            '            <a href tabindex="50" ng-click="ctrl.viewEndpoint(stat)" ' +
	            '               ng-if="stat.statUrl">' +
	            '              <span class="text-ellipsis">{{ ::stat.statName }}</span>' +
	            '            </a>' +
	            '            <span class="text-ellipsis" ng-if="!stat.statUrl">' +
	            '              {{ ::stat.statName }}</span>' +
	            '          </div>' +
	            '          <div class="col-sm-5 text-right">' +
	            '             <span class="text-ellipsis">{{ ::stat.statCount }}</span>' +
	            '          </div>' +
	            '        </div>' +
	            '      </div>' +
	            '    </div>' +
