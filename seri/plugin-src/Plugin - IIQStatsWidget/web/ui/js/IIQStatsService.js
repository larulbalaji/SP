 ...
 */
factory('IIQStatsService', ['SP_CONTEXT_PATH', 'ListResultDTO', 'IIQStats', '$http', function(SP_CONTEXT_PATH, ListResultDTO, IIQStats, $http) {
     ...
     */
    svc.getStats = function(searchTerm, start, limit) {
        var params = {
            start: start,
            limit: limit
        };
        ...
        //console.log(SP_CONTEXT_PATH + '/plugin/rest/iiqstats/stats');
        return $http.get(SP_CONTEXT_PATH + '/plugin/rest/iiqstats/stats', {
            params: params
        }).then(function(response) {
            // Convert the objects in the response into DirectReports.
            var transformed = angular.copy(response.data);
            //console.log(transformed);
            transformed.objects = transformed.objects.map(function(iiqstats) {
                return new IIQStats(iiqstats);
            });
